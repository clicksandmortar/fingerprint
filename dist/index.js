...
function CollectorProvider(_ref) {
  ...
  var setPageTriggers = React__default.useCallback(function (triggers) {
    if (isEqual_1(triggers, pageTriggers)) return;
    setPageTriggersState(triggers);
  }, [setPageTriggersState, pageTriggers]);
  React.useEffect(function () {
    if (intently) return;
  ...
  var TriggerComponent = React__default.useCallback(function () {
    ...
    return activeTriggers.map(function (trigger) {
      ...
      if (potentialComponent && React__default.isValidElement(potentialComponent)) {
        log("CollectorProvider: Potential component for trigger " + trigger.behaviour + " " + trigger.id + " is valid. Mounting");
        return potentialComponent;
      }
      log("CollectorProvider: Potential component for trigger " + trigger.behaviour + " " + trigger.id + " invalid. Running as regular func.");
      return null;
    });
  ...
  var setDisplayedTriggerByInvocation = React__default.useCallback(function (invocation) {
    ...
    if (!invokableTrigger) return;
    if (displayTriggers.includes(invokableTrigger.id)) return;
    setDisplayedTriggers(function (ts) {
      return [].concat(ts, [invokableTrigger.id]);
    });
  }, [pageTriggers, setDisplayedTriggers, displayTriggers]);
  ...
  var fireExitTrigger = React__default.useCallback(function () {
    console.log('TRIED TO FIRE EXIT INTENT TREIGER');
    if (!hasDelayPassed) {
    ...
    startCooldown();
  }, [displayTriggers, hasDelayPassed, canNextTriggerOccur, log, setDisplayedTriggerByInvocation, startCooldown, reRegisterExitIntent, getRemainingCooldownMs]);
  console.log({
  ...
  }, [booted, collect, error, setVisitor, visitor.id, session.id, handlers, initialDelay, getIdleStatusDelay, setIdleTimeout, log, trackEvent, setPageTriggers]);
  ...
  var collectorContextVal = React__default.useMemo(function () {
    return {
      addPageTriggers: setPageTriggers,
