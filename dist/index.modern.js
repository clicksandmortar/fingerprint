import React__default, { createContext, useContext, useState, useEffect, createElement, useCallback } from 'react';
...
import { init as init$1, BrowserTracing, Replay, ErrorBoundary } from '@sentry/react';
...
const init = cfg => {
  mixpanel_cjs.init(MIXPANEL_TOKEN_STAGING, {
    debug: cfg.debug,
    track_pageview: true,
...
const trackEvent = (event, props, callback) => {
  return mixpanel_cjs.track(event, props, callback);
};
const MixpanelProvider = ({
  ...
  useEffect(() => {
    ...
    mixpanel_cjs.identify(visitor.id);
...
const CollectorProvider = ({
  ...
  const showTrigger = displayTrigger => {
    ...
    if (displayTrigger === 'exit') {
      log('CollectorProvider: displayed exit intent, unsubscribing from exit intent for 7 days');
      setCookie('_cm_exit', 'true', 7);
    }
    if (displayTrigger === 'idle') {
      log('CollectorProvider: displayed idle intent, unsubscribing from exit intent for 1 days');
