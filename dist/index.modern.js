...
const FollowContext = React__default.createContext({
  enabled: false
});
const follow = e => {
  const {
...
const FingerprintProvider = ({
  ...
  config: {
    follow: _follow = true,
    consentCallback: _consentCallback = undefined,
    debug: _debug = false,
    defaultHandlers: _defaultHandlers = undefined,
  ...
  const [handlers, setHandlers] = useState(_defaultHandlers || clientHandlers);
  ...
  useEffect(() => {
    ...
    const consentGivenViaCallback = _consentCallback();
