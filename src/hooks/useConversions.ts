...
export const getFuncByOperator = (
  ...
  switch (operator) {
    case 'starts_with':
      return (comparison: string) => comparison.toLowerCase().startsWith(compareWith.toLowerCase());
    case 'contains':
      return (comparison: string) => comparison.toLowerCase().includes(compareWith.toLowerCase());
    case 'ends_with':
      return (comparison: string) => comparison.toLowerCase().endsWith(compareWith.toLowerCase());
    case 'eq':
      return (comparison: string) => comparison.toLowerCase() === compareWith.toLowerCase();
    default:
      return () => {
